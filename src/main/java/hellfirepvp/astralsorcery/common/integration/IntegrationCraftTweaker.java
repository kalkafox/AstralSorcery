...
public class IntegrationCraftTweaker {
    ...
    public static void onCommandCollection(CTCommandCollectionEvent event) {
        ...
        event.registerDump("astralConstellations", "Lists the different Astral Sorcery Constellations", commandContext -> {
            ...
            RegistriesAS.REGISTRY_CONSTELLATIONS.getUserList().forEach(resourceLocation -> {
            ...
            final MutableComponent message = Component.literal(ChatFormatting.GREEN + "Constellations written to the log" + ChatFormatting.RESET);
            commandContext.getSource().customSuggestion(message, true);
            return 0;
        });
        event.registerDump("astralAltarTypes", "Lists the different Astral Sorcery Altar Types", commandContext -> {
            ...
            final MutableComponent message = Component.literal(ChatFormatting.GREEN + "Altar Types written to the log" + ChatFormatting.RESET);
            commandContext.getSource().customSuggestion(message, true);
            return 0;
        });
    }
