...
public class BlockMarblePillar extends BlockMarbleTemplate implements SimpleWaterloggedBlock {
    ...
    private BlockState getThisState(BlockGetter level, BlockPos pos) {
        boolean hasUp   = level.getBlockState(pos.above()).getBlock()   instanceof BlockMarblePillar;
        boolean hasDown = level.getBlockState(pos.below()).getBlock() instanceof BlockMarblePillar;
        if (hasUp) {
            if (hasDown) {
